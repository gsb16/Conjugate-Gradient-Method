...
int main(int argc, char *argv[])
{
	...
	double *md2;			//! Vetor redundante md para efetuar AtA
	...
	simk = (2 * k) - 1;
	simmat = aligned_alloc(ALIGNMENT, ((n * simk * sizeof(double)) + (ALIGNMENT - ((n * simk * sizeof(double)) % ALIGNMENT))));
	res = aligned_alloc(ALIGNMENT, (((n + 1) * sizeof(double)) + (ALIGNMENT - (((n + 1) * sizeof(double)) % ALIGNMENT))));
	rresid = aligned_alloc(ALIGNMENT, (((n + 1) * sizeof(double)) + (ALIGNMENT - (((n + 1) * sizeof(double)) % ALIGNMENT))));
	vb = aligned_alloc(ALIGNMENT, (((n + 1) * sizeof(double)) + (ALIGNMENT - (((n + 1) * sizeof(double)) % ALIGNMENT))));
	atvb = aligned_alloc(ALIGNMENT, (((n + 1) * sizeof(double)) + (ALIGNMENT - (((n + 1) * sizeof(double)) % ALIGNMENT))));
	vetx = aligned_alloc(ALIGNMENT, (((n + 1) * sizeof(double)) + (ALIGNMENT - (((n + 1) * sizeof(double)) % ALIGNMENT))));
	...
	md = aligned_alloc(ALIGNMENT, ((n * k * sizeof(double)) + (ALIGNMENT - ((n * k * sizeof(double)) % ALIGNMENT))));
	md2 = aligned_alloc(ALIGNMENT, ((n * k * sizeof(double)) + (ALIGNMENT - ((n * k * sizeof(double)) % ALIGNMENT))));

	criaMatrizA(k, n, md, md2);
	criaVetorB(k, n, vb);
	#ifdef DEBUG
		imprimeAb(k, n, md, vb);
	#endif
	...
	tempo = timestamp();
	preparaMatrizA(k, n, md, md2, simmat);
	preparaVetorB(k, n, md2, vb, atvb);
